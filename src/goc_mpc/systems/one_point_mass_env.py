...
class OnePointMassEnv:
    ...
    Observation = np.concatenate([qpos(3), qvel(3)]) -> shape (6,)
    Action = positions for both bodies -> shape (3,), [x1,y1,z1]
    """
    metadata = {"render_modes": ["human", "none"]}

    def __init__(self, mode="teleport", n_substeps=10, xml_str=MJCF_ONE_POINT_MASS):
        ...
        # Indices: qpos = [x1,y1,z1], ctrl = same ordering via position actuators
        self.action_dim = 3
        self.obs_dim = self.model.nq + self.model.nv  # 3 + 3 = 6
