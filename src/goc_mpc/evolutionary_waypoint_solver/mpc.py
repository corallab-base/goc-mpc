...
# static-chain setup on top, see that module). The last four
# (reseed_frac/reseed_rho0/reseed_cv_tol/wp_init_random_frac) are
# SmallContinuousVRPSolver-only -- LamarckianGA's own __init__ has no
# **kwargs sink, so passing one of these under algorithm="lamarckian_al"
# fails loudly with a plain TypeError from that constructor, rather than
...
_ALGO_KWARGS = ("tournament_k", "wp_mut_scale", "outer_iters", "inner_maxiter",
                ...
                "reseed_frac", "reseed_rho0", "reseed_cv_tol", "wp_init_random_frac",
                "route_backend", "max_coupled_structures", "coupled_kwargs", "lazy_kwargs",
                "kinematic_seed", "kinematic_seed_kwargs")
